...
<?php if ($login->passwordResetLinkIsValid() == true) { ?>
		...
		<form class="passwort" method="post" action="/" name="new_password_form">
    		...
    		<input id="user_password_new" class="inputfield" type="password" name="user_password_new" placeholder="Neues Passwort eingeben" pattern=".{6,}" required autocomplete="off"><br>
    		
    		<label class="passwort_label" for="user_password_repeat"><?php echo WORDING_NEW_PASSWORD_REPEAT; ?></label><br>
   			<input id="user_password_repeat" class="inputfield" type="password" name="user_password_repeat" placeholder="Passwort wiederholen" pattern=".{6,}" required autocomplete="off"><br>
...
<?php } else { ?>
		...
		<form  method="post" action="/password_reset.php" name="password_reset_form">
    		...
    		<input id="user_email" class="inputfield" type="text" name="user_email" placeholder="E-Mail" required />
